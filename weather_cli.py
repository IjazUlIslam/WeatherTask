...
logging.basicConfig(filename= "weather_logging.log", filemode="w", level=logging.INFO)
...
def process_csv(file_path, range):
    with open(file_path, 'r') as csv_file:
        ...
        for row_data in all_data[1:]:
            date_time_object = datetime.strptime(row_data[0], '%Y%m%dT%H%M').date()
            if date_time_object > starting_date and date_time_object < ending_date:
